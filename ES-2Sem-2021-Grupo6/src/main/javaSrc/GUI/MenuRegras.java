...
public class MenuRegras extends JFrame {
				...
				try {
					if(!textField_2.getText().equals("")) {
						...
						if( ! ((s[s.length-1].equals("E")) || (s[s.length-1].equals("OU") )) ) {
							...
							lblARegraNo.setText("Não pode adicionar mais proposições! Por favor confirme ou limpe a regra.");
							lblARegraNo.setVisible(true);
						} else {
							lblARegraNo.setVisible(false);
							createThreshold(comboBox_Metrica.getSelectedItem().toString(), comboBox_Sinal.getSelectedItem().toString(), Integer.parseInt(textField_1.getText() ), comboBox_logica.getSelectedItem().toString());
							writeToString();
							panel.removeAll();
							addComponents(panel);
							panel.revalidate();
	...
	private void writeToString() {
		String newS =  textField_2.getText()+ " " + comboBox_Metrica.getSelectedItem()+ " " + comboBox_Sinal.getSelectedItem()+ " " + textField_1.getText() + " " + comboBox_logica.getSelectedItem() ;
